...
AeronavicsFlyViewOptions::AeronavicsFlyViewOptions(AeronavicsOptions* options, QObject* parent)
...
bool AeronavicsFlyViewOptions::showMultiVehicleList(void) const
{
    return true;
}

// This custom build has it's own custom instrument panel. Don't show regular one.
bool AeronavicsFlyViewOptions::showInstrumentPanel(void) const
{
    return false;
}

AeronavicsOptions::AeronavicsOptions(AeronavicsPlugin*, QObject* parent)
    : QGCOptions(parent)
{
}

QGCFlyViewOptions* AeronavicsOptions::flyViewOptions(void)
{
    if (!_flyViewOptions) {
        _flyViewOptions = new AeronavicsFlyViewOptions(this, this);
    }
    return _flyViewOptions;
}

// Firmware upgrade page is only shown in Advanced Mode.
bool AeronavicsOptions::showFirmwareUpgrade() const
{
    return qgcApp()->toolbox()->corePlugin()->showAdvancedUI();
}

bool AeronavicsOptions::checkFirmwareVersion() const
{
    return false;
}
...
bool AeronavicsOptions::wifiReliableForCalibration(void) const
{
    return true;
}

AeronavicsPlugin::AeronavicsPlugin(QGCApplication *app, QGCToolbox* toolbox)
    : QGCCorePlugin(app, toolbox)
{
    _options = new AeronavicsOptions(this, this);
    _showAdvancedUI = false;
}

AeronavicsPlugin::~AeronavicsPlugin()
{
}

void AeronavicsPlugin::setToolbox(QGCToolbox* toolbox)
{
    QGCCorePlugin::setToolbox(toolbox);

    // Allows us to be notified when the user goes in/out out advanced mode
    connect(qgcApp()->toolbox()->corePlugin(), &QGCCorePlugin::showAdvancedUIChanged, this, &AeronavicsPlugin::_advancedChanged);
}

void AeronavicsPlugin::_advancedChanged(bool changed)
{
...
//-----------------------------------------------------------------------------
void AeronavicsPlugin::_addSettingsEntry(const QString& title, const char* qmlFile, const char* iconFile)
{
    Q_CHECK_PTR(qmlFile);
...
//-----------------------------------------------------------------------------
QVariantList&
AeronavicsPlugin::settingsPages()
{
...
QGCOptions* AeronavicsPlugin::options()
{
    return _options;
}

QString AeronavicsPlugin::brandImageIndoor(void) const
{
    return QStringLiteral("/custom/img/AeronavicsFullLogoWhite.png");
}

QString AeronavicsPlugin::brandImageOutdoor(void) const
{
    return QStringLiteral("/custom/img/AeronavicsFullLogoBlack.png");
}

bool AeronavicsPlugin::overrideSettingsGroupVisibility(QString name)
{
...
bool AeronavicsPlugin::adjustSettingMetaData(const QString& settingsGroup, FactMetaData& metaData)
{
...
void AeronavicsPlugin::paletteOverride(QString colorName, QGCPalette::PaletteColorInfo_t& colorInfo)
{
...
QQmlApplicationEngine* AeronavicsPlugin::createQmlApplicationEngine(QObject* parent)
{
    QQmlApplicationEngine* qmlEngine = QGCCorePlugin::createQmlApplicationEngine(parent);
    qmlEngine->addImportPath("qrc:/Aeronavics/Widgets");
    return qmlEngine;
}
